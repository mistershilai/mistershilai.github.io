...
export default function Planning() {
  ...
  const [scenario, _setScenario] = useState("2526");
  ...
  const removeFacility = async (name: string, replacement?: string) => {
    ...
    try {
      ...
      api.getRegionDemand(region, scenario).then(setDemandData);
  ...
  useEffect(() => {
    ...
    setInventory({});
    setLastDemand({});
    api.getRegionDemand(region, scenario).then(setDemandData).catch(console.error);
    api.getRegionFacilities(region).then((facs) =>
      setRegionFacilities(facs.map((f) => ({ name: f.name, type: f.type, latitude: f.latitude, longitude: f.longitude })))
    ).catch(console.error);
  }, [region, scenario]);
