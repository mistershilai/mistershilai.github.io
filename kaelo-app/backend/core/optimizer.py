...
def run_simulation(
    ...
    # Build CVXPY problem once; use Parameter for inventory to re-solve
...
def run_planning(
    ...
    # Parse realized demand (this cycle's order request) for post-hoc evaluation.
    # LP does NOT see this; it only plans against the forecast mu.
    realized_np = None
    if last_demand:
        realized_df = pd.DataFrame(0.0, index=nodes, columns=classes)
        for fac, drugs in last_demand.items():
            if fac not in realized_df.index:
                continue
            for drug, qty in drugs.items():
                if drug in realized_df.columns:
                    realized_df.loc[fac, drug] = float(qty)
        realized_np = realized_df.to_numpy().astype(float)
    ...
    if realized_np is not None:
        ...
        if strategy == "adr":
            try:
                alpha_val = np.asarray(alpha.value, dtype=float)
                xi_real = realized_np - mu_np  # observed deviation per (node, drug)
                # Affine rule from paper: F_{(u,v),k}(ξ) = F̄_{(u,v),k} + α_{(u,v),k} · ξ_{v,k}
                # ξ indexed by destination node v for arc (u, v).
                dest_xi = xi_real[arc_dest_idx, :]  # (m, K)
                ship_r = np.maximum(F_val + alpha_val * dest_xi, 0.0)
            except Exception:
                ship_r = F_val
        inflow_r = A_in @ ship_r
        outflow_r = A_out @ ship_r
        supply_r = np.zeros((N, K))
        supply_r[cms_idx, :] = q_val
        avail_r = I0_np + supply_r + inflow_r - outflow_r
